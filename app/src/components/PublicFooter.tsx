    ...
    <footer className="border-t border-[#102015]/10 bg-[#102015] text-white">
      <div className="mx-auto max-w-7xl px-6 py-5 lg:px-10">
        <nav className="flex flex-wrap gap-x-4 gap-y-2">
          {footerLinks.map(([label, href]) => (
            <Link
              key={label}
              href={href}
              className="text-xs font-bold text-white/65 transition hover:text-[#9ee6ad]"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="mt-4 border-t border-white/10 pt-3">
          <div className="grid gap-1 text-[0.68rem] font-medium leading-5 text-white/35 md:grid-cols-[1fr_auto] md:items-end">
            <div>
              <p>
