...
const keyFor = (title: string) =>
  `oft-admin-sidebar-group-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;

export default function AdminSidebarGroup({
  title,
  links,
}: {
  title: string;
  ...
  const groupActive = links.some((link) =>
    isAdminNavigationLinkActive(link, pathname, search),
  );
  const storageKey = keyFor(title);
  ...
  }, [groupActive, storageKey]);

  function handleToggle() {
    const next = Boolean(detailsRef.current?.open);
    setOpen(next);
  ...
  return (
    <details
      ...
      className={`group rounded-2xl border bg-white/[0.03] p-2 ${
        groupActive ? "border-[#9ee6ad]/50" : "border-white/10"
      }`}
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-xl px-2 py-2">
        <p
          className={`text-xs font-black uppercase tracking-[0.22em] ${
            groupActive ? "text-white" : "text-[#9ee6ad]"
          }`}
        >
          {title}
        </p>
            ...
            >
              ...
              <p
                className={`mt-1 line-clamp-2 text-xs leading-5 ${
                  active ? "text-[#173c24]" : "text-white/55"
                }`}
              >
                {item.description}
              </p>
