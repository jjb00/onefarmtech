...
function badgeClass(status: PrelaunchCheckStatus) {
  ...
  if (status === "warn") {
    return "bg-[#F2B84B]/20 text-[#101712]";
  }
    ...
    <main className="space-y-8">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-7 text-[#1E2420]/70">
          This page prevents accidental deployment assumptions. The app can keep
          progressing locally, but Vercel team testing should wait until database
          and authentication foundations are production-ready.
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/65">
              ...
              blocked items are resolved and Supabase/Postgres/auth are deliberately connected.
            </p>
          </div>
      ...
      <section className="grid gap-4">
        {readiness.checks.map((check) => (
          <article key={check.title} className="rounded-[1.5rem] bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-black text-[#101712]">{check.title}</h2>
                <p className="mt-2 text-sm leading-7 text-[#1E2420]/70">
                  {check.detail}
                </p>
              </div>

              <span className={`rounded-full px-3 py-1 text-xs font-black ${badgeClass(check.status)}`}>
                {label(check.status)}
              </span>
            </div>
      ...
      <section className="rounded-[1.5rem] bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-black text-[#101712]">Next safe path</h2>
        <ol className="mt-5 grid gap-3 text-sm leading-7 text-[#1E2420]/75">
          ...
          <li>4. Move Prisma provider to Postgres and test locally against Supabase.</li>
          <li>5. Replace temporary staff password gate with real staff/buyer auth.</li>
