...
export default async function BuyerAccountRequestsPage({searchParams, embedded}: PageProps) {
  ...
  const q = value(raw?.q); const status = value(raw?.status); const source = value(raw?.source); const buyerType = value(raw?.type); const conversion = value(raw?.conversion); const queue = value(raw?.queue) || "active";
  ...
  const params = {view: "applications", q, status, source, type: buyerType, conversion, queue, pageSize};
  const queueWhere =
    status
      ? {status}
      : queue === "active"
        ...
        : queue === "converted"
          ...
          : queue === "rejected"
            ? {status: "Rejected"}
            : queue === "archive"
              ? {status: {in: ["Rejected", "Closed", CONVERTED]}}
              : {};
  ...
  if (page > totalPages) redirect(adminListHref("/admin/customers", params, {page: totalPages}));
  ...
  const range = adminResultRange(page, pageSize, total); const filtered = Boolean(q || status || source || buyerType || conversion || queue !== "active");
  ...
  return <AdminPageShell title="Account requests" description="Review recurring-buyer applications before customer setup and access management." compactHeader embedded={embedded}>
    <div className="grid gap-5">
      ...
      <nav aria-label="Application queues" className="flex flex-wrap gap-2">
        <WorkflowLink href="/admin/customers?view=applications&queue=active" label="Active" />
        <WorkflowLink href="/admin/customers?view=applications&queue=converted" label="Converted" />
        <WorkflowLink href="/admin/customers?view=applications&queue=rejected" label="Rejected" />
        <WorkflowLink href="/admin/customers?view=applications&queue=archive" label="Archive" />
        <WorkflowLink href="/admin/customers?view=applications&queue=all" label="All" />
      </nav>
      <AdminListToolbar search={q} searchLabel="Search applications" searchPlaceholder="Applicant, organisation, phone, registration or notes" pageSize={pageSize} resetHref="/admin/customers?view=applications&queue=active" filters={[
      ...
      {!requests.length ? <AdminEmptyState title={allTotal === 0 ? "No account requests yet" : "No matching account requests"} description={allTotal === 0 ? "New buyer applications will appear here." : "Try a different search term or clear one or more filters."} resetHref={filtered ? "/admin/customers?view=applications&queue=active" : undefined} /> : <>
        <section className="hidden overflow-hidden rounded-2xl border border-[#102015]/10 bg-white shadow-sm md:block"><div className="overflow-x-auto"><table className="w-full min-w-[1000px] border-collapse text-left text-sm"><thead className="bg-[#f3f8ef] text-xs uppercase tracking-[0.12em] text-[#405348]"><tr><th className="px-4 py-3">Applied</th><th className="px-4 py-3">Applicant</th><th className="px-4 py-3">Business</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Conversion</th><th className="px-4 py-3">Review</th></tr></thead><tbody>{requests.map((request) => <RequestRow key={request.id} request={request} customer={customers.get(customerIdFromRequest(request))} canDelete={staff.role === "Super admin"} />)}</tbody></table></div></section>
        <section className="grid gap-3 md:hidden">{requests.map((request) => <RequestCard key={request.id} request={request} customer={customers.get(customerIdFromRequest(request))} canDelete={staff.role === "Super admin"} />)}</section>
      </>}
      <AdminPagination page={page} totalPages={totalPages} previousHref={page > 1 ? adminListHref("/admin/customers", params, {page: page - 1}) : undefined} nextHref={page < totalPages ? adminListHref("/admin/customers", params, {page: page + 1}) : undefined} />
    </div>
  </AdminPageShell>;
}
