...
export default async function OrdersPage({searchParams}: {searchParams?: Promise<Params>}) {
  ...
  if (view === "new-requests") return <NewRequestsView q={q} page={page} pageSize={pageSize} />;
  const where = {
    AND: [
      viewWhere(view),
      ...(q ? [{OR: [
  ...
  const labels = [
    ["needs-action", "Needs action"], ["new-requests", "New requests"],
    ["fulfilment", "In fulfilment"], ["completed", "Completed"],
  ];
  ...
  return (
    <AdminShell title="Orders" description="Review requests, payments and fulfilment." compactHeader action={<Link href="/admin/create-order" className="rounded-full bg-[#1f7a3f] px-5 py-3 text-sm font-black text-white">New order</Link>}>
...
async function NewRequestsView({q, page, pageSize}: {q: string; page: number; pageSize: number}) {
  const where = {
  ...
  const range = adminResultRange(page, pageSize, total);
  return (
    <AdminShell title="Orders" description="Review new order requests." compactHeader action={<Link href="/admin/create-order" className="rounded-full bg-[#1f7a3f] px-5 py-3 text-sm font-black text-white">New order</Link>}>
      <div className="grid gap-5">
