...
import {honeypotIsFilled, intakeFingerprint, registerUniqueIntakeDedupe, validTurnstileResult} from "@/lib/publicIntakeRules.js";
...
async function verifyTurnstile(token: string, action: string, fetcher: typeof fetch) {
  ...
  const expectedHostname = new URL(process.env.APP_BASE_URL || process.env.NEXT_PUBLIC_APP_URL || "https://onefarmtech.com").hostname;
  if (!response.ok || !validTurnstileResult(result, action, expectedHostname)) throw new PublicIntakeError("bot-check");
}
...
export async function protectPublicIntake(input: {formType: "contact" | "order-request"; action: string; token?: string; honeypot?: string; values: unknown[]; fetcher?: typeof fetch}) {
  ...
  await verifyTurnstile(String(input.token || ""), input.action, input.fetcher || fetch);

  const created = await registerUniqueIntakeDedupe(prisma, `${input.formType}:${fingerprint}`, new Date(Date.now() - 10 * 60 * 1000));
