...
## Phase 4 Buyers workspace status

`/admin/customers` is the canonical Buyers workspace with URL-driven All buyers, Guest buyers, Applications, Access and Update requests views. Customer remains the master buyer record; Customer, BuyerAccountRequest, BuyerProfileUpdateRequest, BuyerContact, BuyerAccountInvite and unlinked Order records remain separate.

All buyers now uses database counts, 25/50/100 page sizes, stable updated-time/ID ordering, database search and filters, lightweight order counts and responsive rows/cards. Guest buyers deliberately remains a bounded view of the latest 500 unlinked orders grouped by the existing stored-phone identity and labels that limitation. Applications reuses the completed conversion-integrity queue. Access presents separately paginated contact and invitation sources with a link to the unchanged management actions. Update requests use one shared paginated renderer for standalone and workspace use.

Buyer detail now has Overview, Contacts & access, Orders, Finance, Communications and Activity URL sections. Relation previews are bounded to 25 records and existing account actions remain available. Legacy Buyer accounts, Guest buyers, Applications, Access and Update-request routes remain operational and link to the canonical workspace; they are respectively a compatibility candidate, bounded direct queue, direct queue, contextual management page and direct queue.

Role matrix: Super admin, Admin and Buyer account manager receive all workspace views; Operations receives All buyers, Guests and Applications; Support receives All buyers, Applications and Update requests; Finance receives no buyer-administration workspace access. Proxy path policy and server-side view resolution both apply, while existing actions retain independent authentication.

Applications and Guest views reuse their existing route renderers in embedded mode, while their standalone routes retain the normal admin shell. This keeps one implementation of each queue without nested page chrome. The existing create-buyer action and all legacy conversion, invitation, contact and update actions remain available.

Validation: 43 focused Phase 4, buyer, Communications, list-foundation and conversion-integrity tests passed; all 21 production-foundation tests passed; TypeScript passed; and focused lint completed with zero errors. Global lint remains at the pre-existing baseline of 22 errors and five warnings, including two pre-existing unused-symbol warnings in touched legacy buyer files. The single production build attempt exited 1 because Turbopack could not bind a local process port in the restricted environment (`Operation not permitted`); no application compiler error was reported and the build was not retried.

Known limitations: guest identity has no durable grouping key beyond phone and remains a clearly labelled 500-order sample; Access uses independent contact/invite pagination; no new database indexes were added. Recommended Phase 5 is production query/index observation, a schema-reviewed durable guest identity strategy, and searchable buyer selection for access management if production volumes require it. Operations and Finance consolidation remain out of scope.

## Order and payment integrity phase status

Pickup and delivery fulfilment now use distinct code-level vocabularies without changing the database schema. Pickup begins at `Pending pickup`, may progress through `Confirmed`, `Preparing`, `Ready for pickup` and `Collected`, never requires a Delivery assignment, and cannot be set to `Delivered`. Delivery orders retain delivery-specific assignment and transit states and cannot use pickup labels. Payment remains an independent status: generating a link sets `Payment pending`, while paid status never advances fulfilment and fulfilment never marks payment paid.

`orderStatusRules.js` centralizes the accepted payment and fulfilment vocabularies and validates transitions. Forward operational progress and explicit cancellation are allowed; paid payment state and final fulfilment states reject regressions; delivery exceptions may return only to supported forward delivery recovery states. The existing staff-authorized manual control remains the mutation entry point and now redirects with controlled error codes instead of applying invalid transitions.

Payment initialization is serialized per order with the same PostgreSQL transaction-scoped advisory-lock pattern used by conversion integrity. A valid, unexpired active HTTPS link is reused without another provider call or duplicate notification. Paid requests are never reused. Expired, failed, cancelled, superseded or invalid links may create one fresh uniquely referenced provider attempt; other active attempts are superseded only after the new provider link succeeds. Failed provider initialization persists a failed attempt and returns a controlled error without exposing the provider response. Link generation records audit evidence and creates buyer message/email evidence only for a genuinely new link; it never marks the order paid. Creating a manual payment request also refuses to create another unexpired active request for the same order.

Order-request conversion remains transactionally idempotent and preserves linked-order evidence. Delivery conversions retain `Pending confirmation` payment and `Buyer request` fulfilment states; pickup conversions retain the same independent payment state and begin at `Pending pickup`. Conversion does not create a Delivery record.

Orders, order detail and Payment requests display payment and fulfilment independently. Payment-request rows include delivery method and both order states; expired stored links are labelled unavailable instead of being offered as usable; link actions now reuse or replace according to the integrity service. Order detail still resolves both order ID and legacy order code. No Operations workspace tabs, shared Operations renderers, Finance transition redesign, webhook change, schema change or migration was introduced.

Validation: 35 focused Order/payment, Order-request conversion, Buyers and Communications tests passed; all 21 production-foundation tests passed; changed-file lint, TypeScript and `git diff --check` passed. Global lint remains at the unrelated baseline of 17 errors and five warnings. The single production build attempt exited 1 before compilation because Next could not remove the non-empty existing `.next/server` directory (`ENOTEMPTY`); `.next` was not deleted and the build was not retried. Known limitations are the PostgreSQL-specific advisory lock, free-text status columns, provider expiry depending on stored `expiresAt`, and existing broad staff authorization on legacy admin actions. Phase 5 Operations consolidation has not started and should begin only after this integrity phase is committed.
